// Based on https://code.google.com/p/fakeworker-js/
// A lot of things has to be fixed, including cloning, canceling timeout and interval when the worker was terminated and posting those timeouts on the correct worker.
...
var fakeworker = (function(global){
    ...
    FakeWorker.prototype = {
        ...
        postMessage: function(msg){
            ...
            if (typeof workerContext.onmessage == "function") {
                // for testability, we don't do the "structual clone". (Idiots, not serializing breaks the whole thing)
                var event = new FakeMessageEvent(self);
                var data = structuredClone(msg);
                event.initMessageEvent("message", false, false, data, "", "", null, null);
                workerContext.setTimeout(function(){
                    try {
        ...
        terminate: function(){
            activeWorkers.splice(activeWorkers.indexOf(this), 1);
    ...
    function FakeWorkerContext(url, source, worker) {
        ...
        var globalClearTimeout = global.clearTimeout;
        var globalSetInterval = global.setInterval;
        var globalClearInterval = global.clearInterval;
        ...
        var knownTimeouts = {};
        var knownIntervals = {};
        var setTimeout = this.setTimeout = function(func, delay) {
            if (this.closing) {
                return;
            }

            var id;
            id = globalSetTimeout(function() {
                if (this.closing) {
                    return;
                }
                delete knownTimeouts[id];

                func.apply(null, Array.prototype.slice.call(arguments, 2));
            }, delay);
            knownTimeouts[id] = arguments;
            return id;
        }
        var clearTimeout = this.clearTimeout = function(id) {
            globalClearTimeout(id);
            delete knownTimeouts[id];
        }
        var setInterval = this.setInterval = function(func, delay) {
            if (this.closing) {
                return;
            }

            var id;
            id = globalSetInterval(function() {
                if (this.closing) {
                    return;
                }
            ...
            knownIntervals[id] = arguments;
            return id;
        }
        var clearInterval = this.clearInterval = function(id) {
            globalClearInterval(id);
            delete knownIntervals[id];
        }
        ...
        var close = this.close = function(){
            this.closing = true
            for (var key in knownTimeouts) {
                clearTimeout(key);
            }
            for (var key in knownIntervals) {
                clearInterval(key);
            }
        };
